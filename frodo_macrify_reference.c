...
//    }
//}
void frodo_mul_bs_sf(uint16_t *out,
                     const uint16_t *b,
                     const uint16_t *Sf)
{
    int i, j, k;
    ...
    for (i = 0; i < PARAMS_NBAR; i++) {
        ...
        for (j = 0; j < PARAMS_NBAR; j++) {
            ...
            for (k = 0; k < PARAMS_N; k++) {
                ...
                out[i * PARAMS_NBAR + j] +=
                    b_val * (int16_t)sf_val;

                /*
                 * Only trace output element (0,0)
                 */
                if (i == 0 && j == 0) {
            ...
            out[i * PARAMS_NBAR + j] =
                (uint32_t)out[i * PARAMS_NBAR + j]
                & ((1 << PARAMS_LOGQ) - 1);
        }
    }
}
//For E_row constant
//void frodo_mul_bs_bias(uint16_t *W, const uint16_t *Bp, const uint16_t *S, const uint16_t *E_row) {
//    for (int i = 0; i < PARAMS_NBAR; i++) {
//        for (int j = 0; j < PARAMS_NBAR; j++) {
//            uint32_t sum = 0;
//            for (int k = 0; k < PARAMS_N; k++) {
//                uint16_t bp_biased = (Bp[i*PARAMS_N + k] + E_row[i]) & (PARAMS_Q - 1);
//                sum += bp_biased * (int16_t)S[j*PARAMS_N + k];
//            }
//            W[i*PARAMS_NBAR + j] = sum & (PARAMS_Q - 1);
//        }
//    }
//}

////For E_s (E single value)
//void frodo_mul_bs_bias(uint16_t *W, const uint16_t *Bp, const uint16_t *S, const uint16_t E_s) {
//    for (int i = 0; i < PARAMS_NBAR; i++) {
//        for (int j = 0; j < PARAMS_NBAR; j++) {
//            uint32_t sum = 0;
//            for (int k = 0; k < PARAMS_N; k++) {
//                uint16_t bp_biased = (Bp[i * PARAMS_N + k] + E_s) & (PARAMS_Q - 1);
//                sum += bp_biased * (int16_t)S[j * PARAMS_N + k];
//            }
//
//            W[i * PARAMS_NBAR + j] = sum & (PARAMS_Q - 1);
...
////For E_f (E s fully random value)
//void frodo_mul_bs_bias(uint16_t *W, const uint16_t *Bp, const uint16_t *S, const uint16_t *E_f) {
//    for (int i = 0; i < PARAMS_NBAR; i++) {
//        for (int j = 0; j < PARAMS_NBAR; j++) {
//            uint32_t sum = 0;
//            for (int k = 0; k < PARAMS_N; k++) {
//                uint16_t bp_biased = (Bp[i * PARAMS_N + k] + E_f[i * PARAMS_N + k]) & (PARAMS_Q - 1);
//                sum += bp_biased * (int16_t)S[j * PARAMS_N + k];
//            }
//
//            W[i * PARAMS_NBAR + j] = sum & (PARAMS_Q - 1);
    ...
    for (i = 0; i < PARAMS_NBAR; i++) {
        for (j = 0; j < PARAMS_NBAR; j++) {
            ...
            for (k = 0; k < PARAMS_N; k++) {
                ...
                uint16_t b_val = b[i*PARAMS_N + k];
                int16_t  s_val = (int16_t)s[j*PARAMS_N + k];

                out[i*PARAMS_NBAR + j] += b_val * s_val;
...
//temporary mul-bs_bias
void frodo_mul_bs_bias(
    uint16_t *W,
    const uint16_t *Bp,
    const uint16_t *S,
    const uint16_t *E_row)
{
    for (int i = 0; i < PARAMS_NBAR; i++) {
        for (int j = 0; j < PARAMS_NBAR; j++) {
            ...
            for (int k = 0; k < PARAMS_N; k++) {
                ...
                uint16_t bp_val = Bp[i*PARAMS_N + k];
                int16_t s_val = (int16_t)S[j*PARAMS_N + k];

                uint16_t bp_biased =
                    (bp_val + E_row[i]) & (PARAMS_Q - 1);

                sum += bp_biased * s_val;

                if (i == 0 && j == 0) {
                    printf("MOD,%d,%u,%d,%u,%u,%u\n",
                           k,
                           bp_val,
                           s_val,
                           E_row[i],
